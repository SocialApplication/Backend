  ...
  public health(): Router {
    this.router.get('/health', (req: Request, res: Response) => {
      res
        ...
        .send(`Health: Server instance is healthy with process id ${process.pid} on $`);
    });
  ...
  public fiboRoutes(): Router {
    this.router.get('/fibo/:num', async (req: Request, res: Response) => {
      ...
      res
        .status(HTTP_STATUS.OK)
        .send(
          `Fibonacci series of ${num} is ${result} and it took ${end - start}ms and runs with process id ${process.pid}
          } at}`
        );
    });
